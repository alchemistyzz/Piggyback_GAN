...
LastEditTime: 2023-03-21 11:51:09
FilePath: /zyz/Piggyback_GAN/model/networks.py
...
def define_G(input_nc=3, output_nc=3, ngf=64, netG="resnet_9blocks" , norm='instance', use_dropout=False, init_type='normal', init_gain=0.02, task_num=1, filter_list=[]):
    """Create a generator
...
def define_D(input_nc=3, ndf=64, netD="resnet_9blocks" , norm='instance', init_type='normal', init_gain=0.02, n_layers_D=3):
...
class ResnetGenerator(nn.Module):
    def __init__(self, opt) -> None:
        ...
        input_nc = opt.GENERATOR.INPUT_NC
        output_nc = opt.GENERATOR.OUTPUT_NC
        ngf = opt.GENERATOR.NGF
        k_size = opt.GENERATOR.K_SIZE
        n_downsampling = opt.GENERATOR.N_DOWN
        padding_type = opt.GENERATOR.PADDING_TYPE

        self.replace_conv = replace_conv

        if opt.GENERATOR.NORM == 'batch':
            norm_layer = nn.BatchNorm2d
        elif opt.GENERATOR.NORM == 'instance':
            norm_layer = nn.InstanceNorm2d
        else:
            raise ValueError('NORM type is not supported')
        
        # if opt.GENERATOR.PADDING_TYPE == 'reflect':
        #     padding_type = nn.ReflectionPad2d
        # elif opt.GENERATOR.PADDING_TYPE == 'replicate':
        #     padding_type = nn.ReplicationPad2d
        # elif opt.GENERATOR.PADDING_TYPE == 'zero':
        #     padding_type = nn.ZeroPad2d
        # else:
        #     raise ValueError('PADDING_TYPE type is not supported')

        activation = nn.ReLU(True)

        model = [
            nn.ReflectionPad2d(3),
            nn.Conv2d(input_nc, min(ngf, opt.GENERATOR.MC), kernel_size=7, padding=0),
            norm_layer(ngf),
            activation,
        ]
        # self.f1 = nn.Sequential(*model)
        # model = []
        ### downsample
        for i in range(opt.GENERATOR.START_R):
            mult = 2 ** i
            model += [
                nn.Conv2d(
                    min(ngf * mult, opt.GENERATOR.MC),
                    min(ngf * mult * 2, opt.GENERATOR.MC),
                    kernel_size=k_size,
                    stride=2,
                    padding=1,
                ),
                norm_layer(min(ngf * mult * 2, opt.GENERATOR.MC)),
                activation,
            ]
        for i in range(opt.GENERATOR.START_R, n_downsampling - 1):
            mult = 2 ** i
            model += [
                nn.Conv2d(
                    min(ngf * mult, opt.GENERATOR.MC),
        ...
        if opt.GENERATOR.SPATIO_SIZE == 32:
            model += [
                nn.Conv2d(
                    min(ngf * mult, opt.GENERATOR.MC),
                    min(ngf * mult * 2, opt.GENERATOR.MC),
        ...
        if opt.GENERATOR.SPATIO_SIZE == 64:
            model += [
                ResnetBlock(
                    min(ngf * mult * 2, opt.GENERATOR.MC),
                    padding_type=padding_type,
        ...
        # model += [nn.Conv2d(min(ngf * mult * 2, opt.GENERATOR.MC), min(ngf, opt.GENERATOR.MC), 1, 1)]
        if opt.GENERATOR.FEAT_DIM > 0:
            model += [nn.Conv2d(min(ngf * mult * 2, opt.GENERATOR.MC), opt.GENERATOR.FEAT_DIM, 1, 1)]
        self.encoder = nn.Sequential(*model)

        # decode
        model = []
        if opt.GENERATOR.FEAT_DIM > 0:
            model += [nn.Conv2d(opt.GENERATOR.FEAT_DIM, min(ngf * mult * 2, opt.GENERATOR.MC), 1, 1)]
        # model += [nn.Conv2d(min(ngf, opt.GENERATOR.MC), min(ngf * mult * 2, opt.GENERATOR.MC), 1, 1)]
        ...
        if opt.GENERATOR.SPATIO_SIZE == 32:
            model += [
                nn.ConvTranspose2d(
                    min(ngf * mult, opt.GENERATOR.MC),
                    min(int(ngf * mult / 2), opt.GENERATOR.MC),
                    kernel_size=k_size,
                    stride=2,
                    padding=1,
                    output_padding=o_pad,
                ),
                norm_layer(min(int(ngf * mult / 2), opt.GENERATOR.MC)),
                activation,
            ]
        if opt.GENERATOR.SPATIO_SIZE == 64:
            model += [
        ...
        for i in range(1, n_downsampling - opt.GENERATOR.START_R):
        ...
        for i in range(n_downsampling - opt.GENERATOR.START_R, n_downsampling):
            mult = 2 ** (n_downsampling - i)
            model += [
                nn.ConvTranspose2d(
                    min(ngf * mult, opt.GENERATOR.MC),
        ...
        model += [
            nn.ReflectionPad2d(3),
            nn.Conv2d(min(ngf, opt.GENERATOR.MC), output_nc, kernel_size=7, padding=0),
            nn.Tanh(),
        ]
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, opt, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, getIntermFeat=False):
        ...
        sequence = [[SN(nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw),opt.DISCRIMINATOR.USE_SE), nn.LeakyReLU(0.2, True)]]
        ...
        for n in range(1, n_layers):
            ...
            sequence += [[
                # GaussianNoise(),
                SN(nn.Conv2d(nf_prev, nf, kernel_size=kw, stride=2, padding=padw),opt.DISCRIMINATOR.USE_SE),
        ...
        sequence += [[
            # GaussianNoise(),
            SN(nn.Conv2d(nf_prev, nf, kernel_size=kw, stride=1, padding=padw),opt.DISCRIMINATOR.USE_SE),
            norm_layer(nf),
            nn.LeakyReLU(0.2, True)
        ]]

        sequence += [[SN(nn.Conv2d(nf, 1, kernel_size=kw, stride=1, padding=padw),opt.DISCRIMINATOR.USE_SE)]]
